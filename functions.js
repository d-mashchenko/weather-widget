...
function getWeatherTime(weatherContainer, city) {
  fetch(
    ...
    .then((data) => {
      weatherContainer.getElementsByClassName('weather__city')[0].textContent =
        data.name;
      weatherContainer.getElementsByClassName(
        'weather__humidity'
      )[0].textContent = 'Humidity: ' + data.main.humidity + '%';
      weatherContainer.getElementsByClassName(
        'weather__pressure'
      )[0].textContent = 'Pressure: ' + data.main.pressure + ' hPa';
      weatherContainer.getElementsByClassName('weather__wind')[0].textContent =
      ...
      )[0].textContent = data.weather[0].description;
    })
    .catch((err) => {
  ...
  const date = new Date();
  weatherContainer.getElementsByClassName('weather__date')[0].textContent =
    date.toDateString();
  weatherContainer.getElementsByClassName('weather__time')[0].textContent =
